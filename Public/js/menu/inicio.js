...
async function renderCarouselListado(listado) {
    let plantillaHbs = await fetch('plantillas/marco-carousel.hbs').then(r => r.text())
    var template = Handlebars.compile(plantillaHbs);
    let html = template({ listado })
    document.getElementsByClassName('marco-carousel')[0].innerHTML = html
}
...
function masInfo(e,id) {
    ...
    // acá abro el modal con el elemento seleccionado... detalle-producto.hbs
    initDetalleProducto(producto)
    //deseoController.agregarAlDeseo(producto)    
}
...
async function initInicio() {
    ...
    await renderCarouselListado(productos)
    // document.querySelector('.section-cards__header p').innerHTML = `Se encontraron ${productos.length} productos`
    function getNombreArchivo(id) {
        return 'views/' + id + '.html'
    }
